...
namespace VirtualDeviants.Dialogue.Editor.Utility
{
	...
	{
		private const string StyleDirectory = "Dialogue Author/";
		private const string StyleSuffix = ".uss";
		...
		{
			string name = string.IsNullOrEmpty(hierarchy.parent.name) ? hierarchy.parent.GetType().ToString() : hierarchy.parent.name;
